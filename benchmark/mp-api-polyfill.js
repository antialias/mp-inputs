...
global.MP = {
  // another implementation of MP.api.query just for benchmark since the one in mixpanel-platform
  // doesn't support server side but we need it for extrema.
  api: {
    query: function(endpoint, params) {
      return new JQXHRPromise((resolve, reject) => {
        fetch(`${API_BASE}/${endpoint}?${urlencodeParams(params)}`, {
              ...
              headers: {
                'Authorization': authHeader(global.MP.apiSecret),
              },
        })
        .then(res => res.json())
