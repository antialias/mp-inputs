...
/**
 * Modified by Amanda Canyon
 *   on 3/27/2014
 *
 * Added the pathIncomplete feature to allow for a path to
 * end in a stylized line (default is dashed).
 *
 */
...
defaultOptions = {
  ...
  plotOptions: {
    line: { // base series options
      ...
      events: {},
      incompleteStyle: {'stroke-dasharray': '3,5'},
      //legendIndex: 0,
      //linecap: 'round',
      lineWidth: 2,
      shadow: true,
...
function SVGPathIncomplete() {}

SVGPathIncomplete.prototype = $.extend({}, SVGElement.prototype, {
  init: function (renderer, incompleteStyle) {
    var wrapper = this;
    SVGElement.prototype.init.apply(this, [renderer, 'g']);

    this.children = [new SVGElement(), new SVGElement()];
    this.incompleteStyle = incompleteStyle || {};
    each(this.children, function(element) {
      element.init(renderer, 'path');
      element.add(wrapper);
    });
  ...
  /* Returns an SVG path (in array form) with the last section
   * of the line excluded. dMain + dFinal => graphPath
   *
   * @param {Array} graphPath The full graphPath in array form
   **/
  dMain: function(graphPath) {
    return graphPath.slice(0, -3);
  },

  /* Returns an SVG path (in array form) of the last section
   * of the line.
   *
   * @param {Array} graphPath The full graphPath in array form
   **/
  dFinal: function(graphPath) {
    var path = graphPath.slice(-5);
    path.unshift('M');
    return path;
  },

  shadow: function (shadowOptions, group, cutOff) {
    this.children[0].shadow.apply(arguments);
    this.children[1].shadow.apply(arguments);
    return this;
  },
  ...
  attr: function(hash, val) {
    ...
    // used as a getter: first argument is a string, second is undefined
    if (isString(hash)) {
      return this.children[0].attr(hash, val);
    ...
    // setter
      ...
      delete hash['d'];
      attrMain = $.extend({}, hash);
      attrLast = $.extend({}, hash);

      if (d) {
                // Splitting d (the coordinates of the line) into two: the
                // main line, and the last segment of the line
        extend(attrMain, {d: this.dMain(d)});
        extend(attrLast, {d: this.dFinal(d)});
      }

      extend(attrLast, this.incompleteStyle);
      this.children[0].attr(attrMain);
      this.children[1].attr(attrLast);
  ...
  animate: function (params, options, complete) {
    ...
    this.children[0].animate($.extend({}, params, {d: this.dMain(params.d)}), options, callback);
    this.children[1].animate($.extend({}, params, {d: this.dFinal(params.d)}), options, callback);
  }
...
SVGRenderer.prototype = {
  ...
  /**
   * Draw an incomplete path
   * This path will end with a dotted line
   * @param {Array} path An SVG path in array form
   * @param {Dictionary} incompleteStyle A style applied
   *     to incomplete lines
   */
  pathIncomplete: function (path, incompleteStyle) {
    var element = new SVGPathIncomplete(),
      attr = {
        fill: NONE
      };
    if (isArray(path)) {
    ...
    return element.init(this, incompleteStyle).attr(attr);
  },
...
Series.prototype = {
  ...
  drawGraph: function () {
    var series = this,
      ...
      incompletePathStyle = options.incompleteStyle;
    ...
    each(props, function (prop, i) {
      ...
      if (graph) {
        ...
        if (options.isIncompletePath) {
          series[graphKey] = series.chart.renderer.pathIncomplete(graphPath, incompletePathStyle);
        } else {
